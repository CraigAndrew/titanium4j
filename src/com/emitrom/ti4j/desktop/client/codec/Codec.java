...
public class Codec extends JavaScriptObject {
   ...
   * 
   * @param data,The string to compute the checksum on. The checksum will be
   ...
   *          computed on the UTF-8 version of this String.
   * 
   * @param checksumType,The checksum algorithm to use. Either Codec.CRC32
   *          (default) or Codec.ADLER32
   * @return
   */
   ...
   * @param callback, A function callback that receives the zip file when
   *          writing is finished
   * @return
   */

  public static native String createZip(String root, String zipFile,
      ZipCreateStringDestinationHandler callback)/*-{
		return Ti.Codec
				.createZip(
						root,
						zipFile,
   ...
   * @param root, A directory root to write to the zip stream.
   * @param zipFile, The path or File object of the destination zip file.
   * @param listener, A function callback that receives the zip file when
   *          writing is finished
   * @return
   */

  public static native String createZip(File root, File zipFile,
      ZipCreateFileDestinationHandler listener)/*-{
		return Ti.Codec
				.createZip(
						root,
						zipFile,
						function(destination) {
							callback.@com.emitrom.ti4j.desktop.client.codec.ZipCreateFileDestinationHandler::onComplete(Lcom/emitrom/ti4j/desktop/client/filesystem/file/File;)(destination);
						});
  }-*/;
  ...
  /**
   * Digest a String into a hex binary HMAC. String data will first be converted
   * to UTF-8 data.
   * 
  ...
  /**
   * Encode some data into Base64. String data will first be converted to UTF-8
   * data.
   * 
   * @param data, The data to encode
   * @return
   ...
   * 
   * @param zipFile, The path of the zip file being extracted.
   * @param dest, A directory into which the files will be extracted.
   * @param callback, A function callback that receives destination directory
   *          when completed.
   * @return
   */

  public static native String extractZip(String root, String zipFile,
      ZipExtractStringDestinationHandler callback)/*-{
		return Ti.Codec
				.extractZip(
						root,
						zipFile,
   ...
   * Asynchronously extract the contents of a zip file.
   * 
   * @param zipFile, The File object of the zip file being extracted.
   * @param dest, A directory into which the files will be extracted.
   * @param callback, A function callback that receives destination directory
   *          when completed.
   * @return
   */
  public static native String extractZip(File zipFile, File dest,
      ZipExtractFileDestinationHandler callback)/*-{
		return Ti.Codec
				.extractZip(
						root,
						zipFile,
						function(destination) {
							callback.@com.emitrom.ti4j.desktop.client.codec.ZipExtractFileDestinationHandler::onExtract(Lcom/emitrom/ti4j/desktop/client/filesystem/file/File;)(destination);
						});
  }-*/;
